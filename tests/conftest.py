...
def mock_env_vars(use_real_services):
    ...
    env_vars = {
        "AZURE_STORAGE_ACCOUNT_NAME": os.getenv("AZURE_STORAGE_ACCOUNT_NAME", "teststorage"),
        "AZURE_STORAGE_ACCOUNT_KEY": os.getenv("AZURE_STORAGE_ACCOUNT_KEY", "testkey"),
        "KNOWLEDGE_SEARCH_LOGS_TABLE_NAME": os.getenv("KNOWLEDGE_SEARCH_LOGS_TABLE_NAME", "testlogs"),
        "CONVERSATION_METADATA_TABLE_NAME": os.getenv("CONVERSATION_METADATA_TABLE_NAME", "testmeta"),
        "CONVERSATION_CONTAINER_NAME": os.getenv("CONVERSATION_CONTAINER_NAME", "testconv"),
        "VECTORDB_PATH": os.getenv("VECTORDB_PATH", "/tmp/test.db"),
...
@pytest.fixture
def mock_azure_storage(mock_env_vars, use_real_services):
    ...
    if use_real_services:
        ...
        with patch('app.TableServiceClient') as mock_table_client, \
            ...
            mock_backend_storage.ConversationMetadataStore.return_value = mock_conv_meta_store
            mock_backend_storage.KnowledgeSearchMetadataStore.return_value = mock_knowledge_meta_store
...
def mock_searcher(use_real_services):
    ...
    if use_real_services:
        ...
        with patch('backend.Searching.Searcher') as mock_searcher_class:
            ...
            mock_searcher.all_document_types_table.schema.names = ["id", "content", "type"]
            mock_searcher.all_document_types_table.count_rows.return_value = 1000
            mock_searcher.knowledge_search.return_value = (Mock(), {"total_results": 10, "relevant_results": 5}, {})
...
@pytest.fixture
def client(mock_env_vars, mock_azure_storage, mock_searcher, mock_openai):
...
@pytest.fixture
def mock_assistant(mock_openai, mock_searcher, use_real_services):
